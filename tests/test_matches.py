...
# 3. No Matches Message
@patch("app.mongo")
def test_no_matches_message(mock_mongo, client):
    user_id = str(ObjectId())
    mock_mongo.db.users.find_one.return_value = {"_id": ObjectId(user_id)}
    mock_mongo.db.matches.find.return_value = []
    with client.session_transaction() as sess:
...
# 6. Match Removal Updates Counter (assumes counter on page)
@patch("app.mongo")
